...
export function PlannerResults({ matches, favIds, summary }: PlannerResultsProps) {
  ...
  return (
    <section className="mt-8 animate-fadeUp">
      <div className="mb-4 animate-pop overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-emerald-100 bg-emerald-50 p-5 sm:p-6">
          <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Total budget</p>
      ...
      {matches.length === 0 ? (
        ...
        <>
              ...
              return (
                <div key={d.id} className="relative">
                  <button
                    ...
                    className={`absolute left-3 top-3 z-20 inline-flex min-h-[36px] items-center gap-1 rounded-full px-3.5 py-1.5 text-xs font-semibold shadow-sm backdrop-blur transition active:scale-95 ${
                      isPicked
          ...
          {picked.size > 0 && (
            <div className="sticky bottom-4 mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-lg">
              <div className="flex flex-wrap items-end gap-3">
                ...
                <button
                  ...
                  onClick={onSave}
                  disabled={isSaving}
                  className="inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 active:scale-95 disabled:opacity-60"
                >
